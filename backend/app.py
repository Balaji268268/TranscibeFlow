...
def perform_diarization(audio_file_path):
    """
    Speaker diarization using MFCC feature extraction + Agglomerative Clustering.
    ...
    try:
        ...
        # Need at least 2 seconds for meaningful diarization
        if duration < 2.0:
            print("Audio too short for diarization.")
            return None

        # ── 1. Feature Extraction ──────────────────────────────────────────
        window_sec  = 0.5                        # 0.5s analysis window
        hop_sec     = 0.25                       # 50% overlap
        window_len  = int(window_sec * sr)
        hop_len     = int(hop_sec    * sr)

        # MFCC (13 coefficients) — robust speaker identity features
        ...
        # Transpose → (n_frames, 13), then normalise
        features = mfcc.T
        ...
        max_speakers = min(4, n_frames // 4)   # can't have more clusters than frames/4

        for n in range(2, max_speakers + 1):
            model = AgglomerativeClustering(n_clusters=n, linkage='ward')
            labels = model.fit_predict(features)
            # Mean intra-cluster distance (lower = tighter clusters = better fit)
            score = 0.0
            for c in range(n):
                cluster_pts = features[labels == c]
                if len(cluster_pts) > 1:
                    centroid = cluster_pts.mean(axis=0)
                    score += np.mean(np.linalg.norm(cluster_pts - centroid, axis=1))
            score /= n
            if score < best_score:
                best_score = score
                best_n     = n

        # ── 3. Final Clustering with best_n speakers ───────────────────────
        ...
        frame_times = librosa.frames_to_time(
            np.arange(n_frames), sr=sr, hop_length=hop_len
        )
        ...
        if not segments:
            print("No valid segments found after filtering.")
            return None
        ...
        import traceback
        traceback.print_exc()
        return None  # Graceful fallback — transcription still works
...
def upload_file():
    ...
    try:
        ...
        # 4. Key Highlights — TF-IDF extractive sentence scoring on transcript
        # Completely separate from the BART summary: picks the most information-dense
        # sentences directly from the transcript based on term importance.
        ...
        def extract_key_highlights(text, n=3):
            ...
            try:
                ...
                tfidf_matrix = vec.fit_transform(sents)  # (n_sents, n_features)
        ...
        # 6. Save to History — skip for trial mode uploads
