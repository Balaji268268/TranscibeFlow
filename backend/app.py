...
# Global cached diarization pipeline — loaded once, reused on every upload
_diarization_pipeline = None

def get_diarization_pipeline():
    """Load and cache the pyannote diarization pipeline (loads once at first use)."""
    ...
    try:
        print("Loading diarization pipeline (first time, may take a moment)...")
        _diarization_pipeline = Pipeline.from_pretrained(
            "pyannote/speaker-diarization-3.1",
            use_auth_token="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"
        )
        print("Diarization pipeline loaded successfully.")
        return _diarization_pipeline
    except Exception as e:
        print(f"Failed to load diarization pipeline: {e}")
        print("Tip: Make sure you have accepted the model terms at https://huggingface.co/pyannote/speaker-diarization-3.1")
        return None
...
def perform_diarization(audio_file_path):
    ...
    Returns a dict with num_speakers + segments, or None if single speaker / unavailable.
    """
    ...
    try:
        ...
        # Load via librosa to bypass torchcodec issues on Windows
        waveform_np, sample_rate = librosa.load(audio_file_path, sr=16000, mono=True)

        # pyannote expects: float32 tensor of shape (channels, samples)
        waveform_tensor = torch.from_numpy(waveform_np).float().unsqueeze(0)  # (1, T)
        ...
        # Single speaker — no labelling needed
